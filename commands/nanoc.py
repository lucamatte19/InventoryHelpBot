...
async def check_cooldown_nanoc(user_id: int) -> bool:
    now = time.time()
    last_time = nanoc_times.get(user_id, 0)
    return now - last_time >= COOLDOWNS["nanoc"]
...
async def handle_nanoc_mention(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    item_name = "Nanoc"
    max_cooldown_seconds = COOLDOWNS["nanoc"]

    if time_str:  # Modify existing timer
        print(f"[{item_name}] Modify request detected. Time string: {time_str}")
        
        if user.id not in active_nanoc_tasks:
            await update.message.reply_text(
                f"@{username}, non c'è una nanostruttura a combustione attiva da modificare. Inizia prima con `/usa nanoc`."
            )
        ...
        try:
            ...
            await cancel_active_task(user.id, active_nanoc_tasks, item_name)

            await update.message.reply_text(
                f"@{username}, timer nanoc modificato! Ti avviserò appena pronto."
            )
            if user.id not in disabled_nanoc:
                new_task = asyncio.create_task(notifica_ritorno_nanoc_ridotto(update, user.id, username, total_seconds))
                active_nanoc_tasks[user.id] = new_task
        ...
        if not await check_cooldown_nanoc(user.id):
            now = time.time()
            last_start_time = nanoc_times.get(user.id, 0)
            if last_start_time > 0:
                ...
                await update.message.reply_text(
                    f"@{username}, nanoc in cooldown! Prossimo nanoc disponibile tra: {remaining_time_str}"
                )
            else:
                await update.message.reply_text(f"@{username}, nanoc in cooldown!")
        ...
        update_player_stats(user.id, "nanoc")
        update_last_timer(user.id, "nanoc", time.time())
        
        # Aggiorna statistiche in memoria (retrocompatibilità)
        if user.id not in user_stats:
            user_stats[user.id] = {}
        if "nanoc" not in user_stats[user.id]:
            user_stats[user.id]["nanoc"] = {"today": 0, "total": 0}
        user_stats[user.id]["nanoc"]["today"] = user_stats[user.id]["nanoc"].get("today", 0) + 1
        user_stats[user.id]["nanoc"]["total"] = user_stats[user.id]["nanoc"].get("total", 0) + 1

        # Verifica se l'utente vuole ricevere notifiche
        notifications_enabled = get_notification_status(user.id, "nanoc")
        
        # Se le notifiche sono disattivate, non rispondere affatto
        if user.id in disabled_nanoc or not notifications_enabled:
        ...
        await update.message.reply_text(
            f"{user.mention_html()}, nanoc utilizzato! Ti avviserò appena pronto.",
            parse_mode="HTML"
        )
        task = asyncio.create_task(notifica_ritorno_nanoc(update, user.id, username))
        active_nanoc_tasks[user.id] = task
        print(f"[{item_name}] Scheduled standard notification task for user {user.id}")

async def notifica_ritorno_nanoc(update: Update, user_id: int, username: str):
    sleep_duration = COOLDOWNS["nanoc"]
    task_id = asyncio.current_task().get_name() if hasattr(asyncio.current_task(), 'get_name') else id(asyncio.current_task())
    print(f"[Nanoc Default Task {task_id}] User {user_id}: Sleeping for {sleep_duration} seconds ({format_remaining_time(sleep_duration)}).")
    try:
        await asyncio.sleep(sleep_duration)
    except asyncio.CancelledError:
        print(f"[Nanoc Default Task {task_id}] User {user_id}: Task cancelled during sleep.")
        raise

    print(f"[Nanoc Default Task {task_id}] User {user_id}: Woke up after {sleep_duration} seconds.")
    
    current_active_task = active_nanoc_tasks.get(user_id)
    if not current_active_task or current_active_task != asyncio.current_task():
        print(f"[Nanoc Default Task {task_id}] User {user_id}: Task is no longer active or has been replaced. Exiting.")
        return

    try:
        print(f"[Nanoc Default Task {task_id}] User {user_id}: Sending notification.")
        message = f"@{username}, puoi usare di nuovo nanoc!\n/usa nanoc"
        await send_notification(update, user_id, username, message, "Nanoc")
    except Exception as e:
        print(f"[Nanoc Default Task {task_id}] Error sending notification for user {user_id}: {e}")
    finally:
        if user_id in active_nanoc_tasks and asyncio.current_task() == active_nanoc_tasks.get(user_id):
             print(f"[Nanoc Default Task {task_id}] User {user_id}: Cleaning up active task entry.")
             active_nanoc_tasks.pop(user_id, None)

async def notifica_ritorno_nanoc_ridotto(update: Update, user_id: int, username: str, total_seconds: int):
    sleep_duration = total_seconds
    item_name = "Nanostruttura a combustione"
    task_id = asyncio.current_task().get_name() if hasattr(asyncio.current_task(), 'get_name') else id(asyncio.current_task())
    ...
    try:
        ...
        past_time = time.time() - COOLDOWNS["nanoc"] - 1  # -1 per sicurezza
        nanoc_times[user_id] = past_time
        print(f"[{item_name} Reduced Task {task_id}] User {user_id}: Updated timestamp to allow immediate restart.")
        
        message = f"@{username}, puoi usare di nuovo la nanostruttura a combustione!\n/usa nanoc"
        await send_notification(update, user_id, username, message, "Nanoc")
    except Exception as e:
...
async def toggle_nanoc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"[Nanoc] Toggle command received for user {user_id} (@{username})")
    ...
    notifications_enabled = get_notification_status(user_id, "nanoc")
    print(f"[Nanoc] Current notification status: {'enabled' if notifications_enabled else 'disabled'}")
    
    # Aggiorna stato notifiche (manteniamo anche il vecchio sistema per retrocompatibilità)
    if notifications_enabled:
        disabled_nanoc.add(user_id)
        update_player_notification_setting(user_id, "nanoc", False)
        await update.message.reply_text(
            f"@{username}, ho disattivato le notifiche per nanoc."
        )
        print(f"[Nanoc] Notifications disabled for user {user_id}")
    else:
        if user_id in disabled_nanoc:
            disabled_nanoc.remove(user_id)
        update_player_notification_setting(user_id, "nanoc", True)
        await update.message.reply_text(
            f"@{username}, ho riattivato le notifiche per nanoc."
        )
        print(f"[Nanoc] Notifications enabled for user {user_id}")
