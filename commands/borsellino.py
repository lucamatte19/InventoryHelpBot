...
async def check_cooldown_borsellino(user_id: int) -> bool:
    now = time.time()
    last_time = borsellino_times.get(user_id, 0)
    return now - last_time >= COOLDOWNS["borsellino"]
...
async def handle_borsellino_mention(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    item_name = "Borsellino"
    max_cooldown_seconds = COOLDOWNS["borsellino"]

    if time_str:  # Modify existing timer
        print(f"[{item_name}] Modify request detected. Time string: {time_str}")
        
        if user.id not in active_borsellino_tasks:
            await update.message.reply_text(
                f"@{username}, non c'è un borsellino attivo da modificare. Inizia prima con `/usa borsellino`."
            )
        ...
        try:
            ...
            if not total_seconds:
                await update.message.reply_text(
                    f"@{username}, formato ora non valido. Usa mm:ss o hh:mm:ss."
                )
                return
            ...
            await cancel_active_task(user.id, active_borsellino_tasks, item_name)

            await update.message.reply_text(
                f"@{username}, timer borsellino modificato! Ti avviserò appena pronto."
            )
            ...
            if user.id not in disabled_borsellino and notifications_enabled:
                new_task = asyncio.create_task(notifica_ritorno_borsellino_ridotto(update, user.id, username, total_seconds))
                active_borsellino_tasks[user.id] = new_task
        ...
        if not await check_cooldown_borsellino(user.id):
            now = time.time()
            last_start_time = borsellino_times.get(user.id, 0)
            if last_start_time > 0:
                ...
                await update.message.reply_text(
                    f"@{username}, borsellino in cooldown! Prossimo borsellino disponibile tra: {remaining_time_str}"
                )
            else:
                await update.message.reply_text(f"@{username}, borsellino in cooldown!")
        ...
        borsellino_times[user.id] = time.time()
        daily_stats["borsellino"] += 1
        ...
        update_player_stats(user.id, "borsellino")
        update_last_timer(user.id, "borsellino", time.time())
        ...
        if user.id in disabled_borsellino or not notifications_enabled:
        ...
        await update.message.reply_text(
            f"{user.mention_html()}, borsellino utilizzato! Ti avviserò appena pronto.",
            parse_mode="HTML"
        )
        task = asyncio.create_task(notifica_ritorno_borsellino(update, user.id, username))
        active_borsellino_tasks[user.id] = task
        print(f"[{item_name}] Scheduled standard notification task for user {user.id}")

async def notifica_ritorno_borsellino(update: Update, user_id: int, username: str):
    sleep_duration = COOLDOWNS["borsellino"]
    task_id = asyncio.current_task().get_name() if hasattr(asyncio.current_task(), 'get_name') else id(asyncio.current_task())
    print(f"[Borsellino Default Task {task_id}] User {user_id}: Sleeping for {sleep_duration} seconds ({format_remaining_time(sleep_duration)}).")
    try:
        await asyncio.sleep(sleep_duration)
    except asyncio.CancelledError:
        print(f"[Borsellino Default Task {task_id}] User {user_id}: Task cancelled during sleep.")
        raise

    print(f"[Borsellino Default Task {task_id}] User {user_id}: Woke up after {sleep_duration} seconds.")
    
    current_active_task = active_borsellino_tasks.get(user_id)
    if not current_active_task or current_active_task != asyncio.current_task():
        print(f"[Borsellino Default Task {task_id}] User {user_id}: Task is no longer active or has been replaced. Exiting.")
        return

    try:
        print(f"[Borsellino Default Task {task_id}] User {user_id}: Sending notification.")
        message = f"@{username}, puoi usare di nuovo il borsellino!\n/usa borsellino"
        await send_notification(update, user_id, username, message, "Borsellino")
    except Exception as e:
        print(f"[Borsellino Default Task {task_id}] Error sending notification for user {user_id}: {e}")
    finally:
        if user_id in active_borsellino_tasks and asyncio.current_task() == active_borsellino_tasks.get(user_id):
             print(f"[Borsellino Default Task {task_id}] User {user_id}: Cleaning up active task entry.")
             active_borsellino_tasks.pop(user_id, None)

async def notifica_ritorno_borsellino_ridotto(update: Update, user_id: int, username: str, total_seconds: int):
    ...
    try:
        ...
        past_time = time.time() - COOLDOWNS["borsellino"] - 1  # -1 per sicurezza
        borsellino_times[user_id] = past_time
...
async def toggle_borsellino(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"[Borsellino] Toggle command received for user {user_id} (@{username})")
    ...
    notifications_enabled = get_notification_status(user_id, "borsellino")
    print(f"[Borsellino] Current notification status: {'enabled' if notifications_enabled else 'disabled'}")
    
    # Aggiorna stato notifiche (manteniamo anche il vecchio sistema per retrocompatibilità)
    if notifications_enabled:
        disabled_borsellino.add(user_id)
        update_player_notification_setting(user_id, "borsellino", False)
        await update.message.reply_text(
            f"@{username}, ho disattivato le notifiche per il borsellino."
        )
        print(f"[Borsellino] Notifications disabled for user {user_id}")
    else:
        if user_id in disabled_borsellino:
            disabled_borsellino.remove(user_id)
        update_player_notification_setting(user_id, "borsellino", True)
        await update.message.reply_text(
            f"@{username}, ho riattivato le notifiche per il borsellino."
        )
        print(f"[Borsellino] Notifications enabled for user {user_id}")
